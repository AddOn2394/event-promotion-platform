...
## ADR-030 — HU-8: quinto estado "Fallida", distinto de "Sin respuesta" y "Rebotada" (corrige ADR-024)

**Decisión**: el listado de invitaciones/confirmaciones (HU-8) pasa de 4 a **5 estados que nunca se agrupan**: `confirmada`, `cancelada`, `sin_respuesta`, `rebotada` y **`fallida`**. `fallida` = la notificación de invitación más reciente quedó en `estado_envio = 'fallido'` (la llamada a Resend falló, o Resend reportó `email.failed`) y la invitación no tiene confirmación.

**Reglas de precedencia** (una sola evaluación, en este orden):
1. Si la invitación tiene confirmación, gana el estado de la confirmación (`confirmada`/`cancelada`), como antes.
2. Si no, `rebotado` → `rebotada`; `fallido` → `fallida`.
3. Todo lo demás (`enviado`, `pendiente`, sin notificación) → `sin_respuesta`. `pendiente` **no** es un estado propio: el envío nace `pendiente` y pasa a `enviado`/`fallido` en segundos, así que mostrarlo como problema sería ruido, y si un proceso muere entre el COMMIT y el marcado queda como "sin respuesta" (sin señal falsa de fallo).

**Por qué**: hallazgo del líder al probar el flujo end-to-end de Gate 7 — una invitación cuyo correo nunca salió (API key de relleno) aparecía como "Sin respuesta", idéntica a un cliente que simplemente no entró. Ventas no podía distinguir "esperar al cliente" de "el correo nunca llegó, hay que reenviar". ADR-024 ya establecía el principio (rebote ≠ sin respuesta); un fallo de envío es el mismo caso y merece el mismo tratamiento. Elegida sobre agrupar `fallido` con `rebotado` (son causas distintas: un rebote es del buzón del cliente, un fallo es del sistema o de la configuración) y sobre un sexto estado para `pendiente` (más superficie de la que HU-8 pide).

**Comportamiento preservado**: el estado se calcula con la notificación de invitación **más reciente** (`LATERAL … ORDER BY creada_en DESC LIMIT 1`), así que un reenvío exitoso (HU-11) crea una notificación nueva y **limpia "Fallida"** sin ninguna lógica adicional. Una invitación con confirmación cuyo envío de invitación falló muestra el estado de la confirmación — si el cliente confirmó, el correo le llegó por algún medio y "Fallida" ocultaría el dato que ventas necesita.

**Impacto**: `EstadoInvitacionAdmin` en `packages/shared-types`, `calcularEstadoInvitacion`, etiquetas y filtro de `InvitacionesPage`/`ConfirmacionesPage`, columna `estado` del CSV (emite el valor crudo del enum), `openapi.json`, `spec/SPEC_FUNCIONAL.md` HU-8. Ninguna migración — `estado_envio` ya admitía `fallido`.

## ADR-031 — Formateador de montos compartido en `packages/shared-types` (extiende ADR-025)

**Decisión**: `formatearCents` vive en `packages/shared-types/src/money.ts` y es la **única** función que convierte centavos a texto para una persona (pantallas de cliente y admin, correos): `Q1,500.00` — coma de miles, punto decimal, sin `Intl`/`toLocaleString`. El export CSV (`centsAQuetzales` en `apps/api/src/admin/service.ts`) queda **fuera**: es un formato de máquina, sin `Q` ni separador.

**Por qué**: ADR-025 admite en `shared-types` funciones puras de *lógica de negocio* sin I/O que ambas apps necesitan idénticas; un formateador es presentación, no negocio, así que estirar ADR-025 en silencio habría sido una decisión tomada sin ratificar. Cumple los dos criterios de admisión (sin DB/HTTP/I/O; web y api necesitan el mismo texto exacto) y hay una razón concreta: el formato ya estaba duplicado a mano (`formatearCents` en `apps/web`, `toFixed(2)` inline en los correos de `apps/api`) y las dos copias no coincidían en criterio. Determinismo explícito porque el resultado no puede variar con el locale del navegador ni con el ICU de Node.

**Precondición**: `cents` es un entero (ADR-005). La función lanza `RangeError` ante un no entero en vez de formatearlo mal — el enfoque por dígitos es exacto para enteros y produce texto sin sentido para un float.

**Impacto**: `apps/web/src/shared/format.ts` eliminado; sus consumidores y las plantillas de correo importan de `@event-promotion/shared-types`. Tests en `money.test.ts` con las fronteras de ADR-005 (`150000` → `Q1,500.00`, `150001` → `Q1,500.01`).
