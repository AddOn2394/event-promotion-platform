...
export async function listarSlotsActivos(): Promise<Slot[]> {
  const { rows } = await pool.query<SlotRow>(
    "SELECT idslot, fecha_hora_inicio, fecha_hora_fin FROM slots WHERE activo = true ORDER BY fecha_hora_inicio",
  );
...
export async function buscarSlotActivoPorId(idslot: string): Promise<Slot | null> {
  const { rows } = await pool.query<SlotRow>(
    "SELECT idslot, fecha_hora_inicio, fecha_hora_fin FROM slots WHERE idslot = $1 AND activo = true",
    [idslot],
  );
  const row = rows[0];
